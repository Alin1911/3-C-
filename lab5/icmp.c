...
void ping(int sockfd, struct in_addr ip_addr, int count)
{
	...
	while(i < count || count == -1)
	{
		...
		printf("bytes :%d;  from:%s;  icmp_seq:%d;     ttl=%f;       time=%f;   \n",ntohs(rip_hdr->tot_len),
			inet_ntoa(ip_addr),ricmp_hdr->un.echo.sequence,time_elapsed);
		i++;
	}
}
...
void traceroute(int sockfd, struct in_addr ip_addr)
{
	...
	int i = 1; //folosit si pentru ttl
	struct in_addr tabelaAddr;// tabela finala cu adresa sursei
	uint8_t raspuns = ICMP_TIME_EXCEEDED; // pentru verificarea raspunzului

	while(i >= 1 || raspuns == ICMP_TIME_EXCEEDED)
	{
        ...
        tabelaAddr.s_addr = rip_hdr->saddr;//am pus adresa in tabela
		raspuns = ricmp_hdr->type;

		printf("%d == %s   timp:%f\n",i, inet_ntoa(tabelaAddr),time_elapsed);
		//am printat indexul adresa si timpul
		i++;
	}
}
