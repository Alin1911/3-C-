...
int receive_and_send(int connfd1, int connfd2)
{
    ...
    while(bytes_remaining > 0){
    	bytes_send = send(connfd2, buf + bytes_received - bytes_remaining, bytes_remaining,0); 
    	DIE(bytes_send <0, "eroare trimitere serv-client");
...
int main(int argc, char* argv[])
{
    ...
    DIE(listenfd < 0,"Eroare creare socket");
