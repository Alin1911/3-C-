...
void create_buf(char *fisier, char* sendbuf){
	int fis;
	char fisiertext[MAXLEN];

	fis = open(fisier, O_RDONLY);
    if (fis < 0){
        printf("Nu pot deschide un fisier");
        return;
    }

	lseek(fis, 0, SEEK_SET);
	read(fis, fisiertext, MAXLEN-1);

    strcpy(sendbuf, "Subject: ex2 \r\nTo: andreialin1@gmail.com\r\nContent-Type: multipart/mixed; boundary=abc\n\r\n--abc\n\r\nContent-Type: text/plain\r\nAtasez tema.\n\r\ncu bine,\r\nStudentul\n\r\n--abc\r\nContent-Type: text/plain\r\nContent-Disposition: attachment; filename=\"");
    strcat(sendbuf,fisier);
    strcat(sendbuf,"\"\n\r\n");
  	strcat(sendbuf,fisiertext);
  	strcat(sendbuf,"--abc\r\n.");
  	close(fis);
}
...
int main(int argc, char **argv) {
    ...
    sprintf(sendbuf, "MAIL FROM: andreialin@test.ro");
  	send_command(sockfd, sendbuf, "250");

    // TODO 5: trimiteti comanda de RCPT TO
    sprintf(sendbuf, "RCPT TO: andreialin1@test.ro");
  	send_command(sockfd, sendbuf, "250");
